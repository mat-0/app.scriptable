...
class Configuration {
    ...
    // logo is downloaded only the first time! It is saved in iCloud and then loaded from there every time afterwards
    logoUrl =
    ...
    if (
        ...
        if (homeBridgeStatus.pluginVersionInfos.updateAvailable) {
            for (plugin of homeBridgeStatus.pluginVersionInfos.plugins) {
                if (
                    CONFIGURATION.pluginsOrSwUpdatesToIgnore.includes(
                        plugin.name
                    )
                ) {
                    continue;
        ...
        addStyledText(
            verticalStack,
            CONFIGURATION.siriGui_title_all_UTD,
            infoFont
        );
    }
}
...
async function buildUsualGui(widget, homeBridgeStatus) {
    ...
    if (cpuData && ramData) {
        ...
        let temperatureString = getTemperatureString(
            cpuData?.cpuTemperature.main
        );
        ...
        let updatedAt = addStyledText(
            widget,
            "t: " + timeFormatter.string(new Date()),
            updatedAtFont
        );
    ...
    if (!CONFIGURATION.adaptToLightOrDarkMode) {
        ...
        switch (CONFIGURATION.bgColorMode) {
            ...
            case "BLACK_DARK":
                setGradient(
                    widget,
                    blackBgGradient_light,
                    blackBgGradient_dark
                );
                break;
            case "PURPLE_DARK":
            case "PURPLE_LIGHT":
            default:
                setGradient(
                    widget,
                    purpleBgGradient_light,
                    purpleBgGradient_dark
                );
        }
    }
}
...
function getChartColorToUse() {
    if (
        CONFIGURATION.adaptToLightOrDarkMode &&
...
async function getAuthToken() {
    if (
        ...
        ">enter the ip with the port here<"
    ) {
...
function getMaxString(arrayOfNumbers, decimals) {
    let factor = Math.pow(10, decimals);
    return (
        Math.round((Math.max(...arrayOfNumbers) + Number.EPSILON) * factor) /
        factor
    )
        .toString()
        .replace(".", CONFIGURATION.decimalChar);
...
function getMinString(arrayOfNumbers, decimals) {
    let factor = Math.pow(10, decimals);
    return (
        Math.round((Math.min(...arrayOfNumbers) + Number.EPSILON) * factor) /
        factor
    )
        .toString()
        .replace(".", CONFIGURATION.decimalChar);
...
function getTemperatureString(temperatureInCelsius) {
    ...
    if (CONFIGURATION.temperatureUnitConfig === "FAHRENHEIT") {
        return (
            getAsRoundedString(convertToFahrenheit(temperatureInCelsius), 1) +
            "°F"
        );
    } else {
...
async function handleNotifications(hbRunning, hbUtd, pluginsUtd, nodeUtd) {
    ...
    if (
        ...
        if (!CONFIGURATION.disableStateBackToNormalNotifications) {
            scheduleNotification(
                CONFIGURATION.notifyText_hbNotRunning_backNormal
            );
        }
    }
    ...
    if (
        ...
        if (!CONFIGURATION.disableStateBackToNormalNotifications) {
            scheduleNotification(
                CONFIGURATION.notifyText_pluginsNotUtd_backNormal
            );
        }
    }

    if (
        shouldNotify(nodeUtd, state.nodeUtd.status, state.nodeUtd.lastNotified)
    ) {
        ...
        if (!CONFIGURATION.disableStateBackToNormalNotifications) {
            scheduleNotification(
                CONFIGURATION.notifyText_nodejsNotUtd_backNormal
            );
        }
    }
...
function shouldNotify(currentBool, boolFromLastTime, lastNotifiedDate) {
    return (
        !currentBool &&
        (boolFromLastTime || isTimeToNotifyAgain(lastNotifiedDate))
    );
}
...
function isTimeToNotifyAgain(dateToCheck) {
    ...
    return (
        timeBetweenDates >
        CONFIGURATION.notificationIntervalInDays * 24 * 60 * 60
    );
}
